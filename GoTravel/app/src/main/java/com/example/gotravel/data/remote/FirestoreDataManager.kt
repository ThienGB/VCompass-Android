...
class FirestoreDataManager () {
    ...
    suspend fun fetchAccommodation(onComplete: () -> Unit) {
        try {
            ...
            if (!result.isEmpty){
                runBlocking {
                    accomDao.deleteAllAccommodation()
                }
            }
            for (document in result) {
                val accommodation = document.toObject<Accommodation>()
                accommodation.accommodationId = document.id
    ...
    suspend fun fetchBooking(onComplete: () -> Unit) {
        try {
            ...
            if (!result.isEmpty) {
                runBlocking {
                    bookingDao.deleteAllBooking() // Chờ xóa xong
                }
            }
            for (document in result) {
                val booking = document.toObject<Booking>()
                bookingDao.insertOrUpdateBooking(booking, onComplete)
            }
        } catch (exception: Exception) {
            Log.w("FirestoreDataManager", "Error getting documents.", exception)
        }
